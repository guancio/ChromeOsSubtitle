(function($) {
	...
	buildsettings: function(player, controls, layers, media) {
		...
		'<div style="color:#fff;margin: auto;position: absolute;top: 0; left: 0; bottom: 0; right: 0;width:650px;display: table; height: auto;background: url(background.png);background: rgba(50,50,50,0.7);border: solid 1px transparent;padding: 10px;overflow: hidden;-webkit-border-radius: 0;-moz-border-radius: 0;border-radius: 0;font-size: 16px;visibility: hidden;">'+
		    '<h2>Settings</h2>' +
		    '<div><ul id="settings_list"></ul></div>' +
		...
		.append($('<label>Default subtitle font size</label>'))
		.append($('<input/>'));
	    $('<li/>')
	    	.appendTo(settingsList)
		.append($('<label>Default opensubtitle.org language</label>'))
		.append($('<select><option>English</option></select>'));
